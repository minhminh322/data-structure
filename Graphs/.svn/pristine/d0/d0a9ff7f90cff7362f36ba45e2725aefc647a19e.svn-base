...
//         System.out.println(myGraph);
        ...
        System.out.println("The number of vertices in the graph : " + myGraph.getNumberOfVertices());
        System.out.println("The number of edges in the graph : " + myGraph.getNumberOfEdges());
        
        
        // find diameter
        System.out.println("The diameter of this graph is : " + calculateDiameter());
        ...
        while (runAgain) { // loops until the user chooses to quit
            ...
            if (from == QUIT_OPTION) {
            	...
            	//	show correct degree for the 'from' city
    ...
    private void displayPathLength(final Vertex<String> thePoint1,
        ...
        double pathLength;
        // set path_length correctly
        
//        WeightedAdjMatrixGraph<String> test = new WeightedAdjMatrixGraph<>();
        
        pathLength = ((WeightedAdjMatrixGraph<String>) myGraph).minimalPath(thePoint1, thePoint2);
	...
	/**
	 * Helper method to calculate diameter of graph.
	 * 
	 * @return double diameter result.
	 */
	private double calculateDiameter() {
		final double[][] shortestArray = WeightedAdjMatrixGraph
				.floydShortestPaths((WeightedAdjMatrixGraph<String>) myGraph);
		final List<Double> shortestList = new ArrayList<>();

		/* Create a list of shortest pairs in graph. */
		for (int row = 0; row < shortestArray.length; row++) {
			for (int col = 0; col < shortestArray.length; col++) {
				shortestList.add(shortestArray[row][col]);
			}
		}
		/* Find the greatest path. */
		double diameter = 0.0;
		for (final Double current : shortestList) {
